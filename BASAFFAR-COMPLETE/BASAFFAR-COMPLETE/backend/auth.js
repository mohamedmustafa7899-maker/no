...
const isPhone    = (s) => /^\+?[\d\s\-(]{7,20}$/.test(s);
const pwError    = (p) => {
...
router.post('/register', registerLimiter, async (req, res) => {
  ...
  if (!isPhone(phone))
    return res.status(400).json({ ok: false, msg: 'يرجى إدخال رقم هاتف صحيح' });
...
router.post('/login', loginLimiter, async (req, res) => {
  ...
  if (!user) {
    addAuditLog(db, 'login_failed', null, { ip: req.ip, extra: { email, reason: 'user_not_found' } });
    writeDB(db);
    return res.status(401).json({ ok: false, msg: 'البريد الإلكتروني أو كلمة المرور غير صحيحة' });
  }
